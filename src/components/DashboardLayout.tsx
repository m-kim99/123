...
import { startInnopayAutopay, cancelInnopaySubscription, resumeInnopaySubscription, PLAN_PRICING, hidePaymentUi, type PaidPlanName } from '@/lib/payments';
...
export function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const changeLanguage = (lng: string) => {
    ...
    if (uid) {
      savePreference(uid, 'language', lng as 'ko' | 'en' | 'ja' | 'de' | 'zh');
    }
  };
            ...
            <DropdownMenuContent align="end" className="w-64">
              ...
              {mobileLangExpanded && (
                <div className="bg-slate-50 border-y">
                  ...
                  <DropdownMenuItem onClick={() => changeLanguage('zh')} className="pl-8">
                    <span className={i18n.language === 'zh' ? 'font-semibold text-[#2563eb]' : ''}>
                      {t('language.chinese')}
                    </span>
                  </DropdownMenuItem>
                </div>
                  ...
                  <DropdownMenuSub>
                    ...
                    <DropdownMenuSubContent>
                      <DropdownMenuRadioGroup value={i18n.language} onValueChange={changeLanguage}>
                        ...
                        <DropdownMenuRadioItem value="zh">{t('language.chinese')}</DropdownMenuRadioItem>
                      </DropdownMenuRadioGroup>
                    </DropdownMenuSubContent>
                  </DropdownMenuSub>
                  <DropdownMenuSub>
                ...
                <div className="space-y-5">
                  ...
                  {isAdmin &&
                    ...
                    <div className="p-4 bg-white border border-slate-200 rounded-lg space-y-3">
                      ...
                      {/* 환불은 고객센터 메일 접수로 처리한다 (앱에서 직접 PG 취소를 호출하지 않음).
                          예약 결제(체험 중)는 아직 결제 건이 없어 환불 대상이 아니다. */}
                      {subscriptionInfo.paymentProvider === 'innopay' &&
                        subscriptionInfo.status === 'active' && (
                        <a
                          href={`mailto:support@traystorage.net?subject=${encodeURIComponent(
                            t('billing.refundMailSubject'),
                          )}`}
                          className="block w-full text-center text-xs text-slate-400 hover:text-slate-600 underline underline-offset-2"
                        >
